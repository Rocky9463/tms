...
include_once 'db.php';
$sql = "SELECT * FROM building_details WHERE email = '$mail'";
...
if(mysqli_fetch_assoc($result) == ''){
    ...
    if ($number == '' && $address == ''){
        $sql = "UPDATE `building_details` SET `building_name`='$b_name', `no_of_rooms`='$rooms' WHERE `email` = '$mail'";
        if (mysqli_query($conn, $sql)) {
            echo "<script>setTimeout(\"location.href = 'http://localhost/tms/managetenant.html';\",1);</script>";
            exit();
          } else {
        ...
        if ($number == ''){
            $sql = "UPDATE `building_details` SET `building_name`='$b_name', `no_of_rooms`='$rooms', `o_address`='$address' WHERE `email` = '$mail'";
            if (mysqli_query($conn, $sql)) {
                echo "<script>setTimeout(\"location.href = 'http://localhost/tms/managetenant.html';\",1);</script>";
                exit();
              } else {
            ...
            $sql = "UPDATE `building_details` SET `building_name`='$b_name', `no_of_rooms`='$rooms', `o_number`='$number' WHERE `email` = '$mail'";
            if (mysqli_query($conn, $sql)) {
                echo "<script>setTimeout(\"location.href = 'http://localhost/tms/managetenant.html';\",1);</script>";
                exit();
              } else {
            ...
            $sql = "UPDATE `building_details` SET `building_name`='$b_name', `no_of_rooms`='$rooms', `o_number`='$number',`o_address`='$address' WHERE `email` = '$mail'";
            if (mysqli_query($conn, $sql)) {
                echo "<script>setTimeout(\"location.href = 'http://localhost/tms/managetenant.html';\",1);</script>";
                exit();
              } else {
